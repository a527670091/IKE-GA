...
2. 演示质量要求：
   ...
   - 保持演示的一致性和清晰性
3. **特异性保护（重点，参考原始IKE论文）**：
   - 在 {k_demos} 条演示中，必须包含至少 60% 的"邻域保护演示"（即 {int(k_demos * 0.6)} 条左右）
...
4. 剩余约 40% 的演示用于保证 ES 和 PS：
   - Copy 类型（约20%）：直接重复新事实
   - Update 类型（约20%）：用改写的问法测试新事实
...
2. **针对 NS 低的核心修复方案（参考原始IKE论文的60%比例）**：
   - 如果你的 NS < 0.3，必须在 {k_demos} 条演示中加入至少 65-70% 的"邻域保护演示"（即 {int(k_demos * 0.65)} 到 {int(k_demos * 0.70)} 条）
   - 如果你的 NS 在 0.3-0.5 之间，加入至少 60% 的"邻域保护演示"（即 {int(k_demos * 0.60)} 条左右）
   - 邻域保护演示定义：
